...
{
    private static final EmotionWeatherService SERVICE = new EmotionWeatherService(0.8, 0.6, 0.4, 0.6);
    ...
    private static EmotionWeatherType invokeMapWeather(double positive, double negative, double anxiety) throws Exception
    {
        ...
        return (EmotionWeatherType) method.invoke(SERVICE, positive, negative, anxiety);
    }
    ...
    //region 精确边界值测试 (已适配 >=)
    //* 下面是 mapWeather 当前使用 >= 的边界值测试.
    //? 如果后续改为严格 >, 这些测试需要同步更新.

    @Test void boundaryExact_AnxietyEq08_ShouldBeThunderstorm() throws Exception
    {
        //* anxiety=0.8 满足 >=0.8, 正确映射为 THUNDERSTORM.
        assertEquals(EmotionWeatherType.THUNDERSTORM, invokeMapWeather(0.5, 0.3, 0.8));
    }

    @Test void boundaryExact_NegativeEq08_ShouldBeThunderstorm() throws Exception
    {
        //* negative=0.8 满足 >=0.8, 正确映射为 THUNDERSTORM.
        assertEquals(EmotionWeatherType.THUNDERSTORM, invokeMapWeather(0.7, 0.8, 0.3));
    }

    @Test void boundaryExact_NegativeEq06_ShouldBeRainy() throws Exception
    {
        //* negative=0.6 满足 >=0.6, 正确映射为 RAINY.
        assertEquals(EmotionWeatherType.RAINY, invokeMapWeather(0.3, 0.6, 0.3));
    }

    @Test void boundaryExact_PositiveEq06_ShouldBeSunny() throws Exception
    {
        //* 边界统一为 >=: positive=0.6 满足 >=0.6, 正确映射为 SUNNY.
        assertEquals(EmotionWeatherType.SUNNY, invokeMapWeather(0.6, 0.4, 0.3));
    }

    @Test void boundaryExact_NegativeEq04_ShouldBeOvercast() throws Exception
    {
        //* negative=0.4 满足 >=0.4, 正确映射为 OVERCAST.
        assertEquals(EmotionWeatherType.OVERCAST, invokeMapWeather(0.3, 0.4, 0.3));
    }
    //endregion
}